...
import { LIKE_TARGET_BOARD_ARTICLE, UPDATE_COMMENT } from '../../apollo/user/mutation';
import { CREATE_COMMENT } from '../../apollo/admin/mutation';
...
const CommunityDetail: NextPage = ({ initialInput, ...props }: T) => {
	...
	const likeBoArticleHandler = async (user: any, id: any) => {
		try {
			...
			console.log('Errorr, likeboArticleHandler:', err.message);
	...
	const updateButtonHandler = async (commentId: string, commentStatus?: CommentStatus.DELETE) => {
		try {
			...
			if (commentStatus) {
				...
				await sweetMixinSuccessAlert('Sucessfully updated!');
			}
			await getCommentsRefetch({ input: searchFilter });
		} catch (error: any) {
			await sweetMixinErrorAlert(error.message);
...
function setCommentTotal(arg0: any) {
	throw new Error('Function not implemented.');
}

function setTotalCount(total: any) {
	throw new Error('Function not implemented.');
}
